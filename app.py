                ...
                if all_data_df is not None and not all_data_df.empty:
                    ...
                    if account_col and amount_col:
                        ...
                        if not bs_df.empty:
                            ...
                            **調査結果:**
                            Excelファイルには「貸･事業所(合計)」シートが存在し、
                            BSデータ（現金預金、売掛金、固定資産等）が完全に入っています。
                            
                            **問題の原因:**
                            インポート処理の`item_mapping`（科目マッピング）にBS科目が
                            含まれていないため、インポート時にBS科目が除外されています。
                            
                            **技術的詳細:**
                            - Excelファイル: ✅ BSシートあり（1番目）
                            - BSデータ: ✅ 完全（現金、売掛金、固定資産等）
                            - インポート処理: ❌ PL科目のみitem_mappingに定義
                            - 結果: BSデータがフィルタリングされて除外
                            """)
                            
                            st.info("""
                            **解決方法:**
                            
                            ### 方法1: data_processorを修正（根本解決）
                            
                            `data_processor.py`の`item_mapping`にBS科目を追加:
                            
                            ```python
                            # BS科目を追加
                            '現金及び預金': ['現金', '当座預金', '普通預金', '定期預金'],
                            '売掛金': ['売掛金'],
                            '棚卸資産': ['商品', '貯蔵品'],
                            '流動資産合計': ['流動資産合計'],
                            '有形固定資産': ['附属設備', '車両運搬具'],
                            ...
                            ### 方法2: 全シート・全科目をインポート（簡単）
                            
                            `import_yayoi_excel`関数で:
                            ```python
                            # item_mappingを使わず、すべての科目を保存
                            ```
                            
                            ### 方法3: 当面の運用
                            
                            PLベースの分析で運用し、BS機能は次のバージョンで対応
                            """)
                    else:
