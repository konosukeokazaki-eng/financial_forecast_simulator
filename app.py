                    ...
                    if account_col and amount_col:
                        ...
                        if not bs_df.empty:
                            ...
                            try:
                                ...
                                LIMIT 3
                                """
                                periods_df = processor._read_sql_query(periods_query, params=())
                                
                                if not periods_df.empty and len(periods_df) >= 1:
                                    period_ids = periods_df['fiscal_period_id'].tolist()
                                    
                                    # 各期のBSデータを取得
                                    bs_comparison_data = []
                                    
                                    for period_id in reversed(period_ids):  # 古い順
                                        period_query = f"""
                                        ...
                                        """
                                        period_data = processor._read_sql_query(period_query, params=())
                                        ...
                                        if not period_data.empty:
                                            ...
                                            for _, row in period_data.iterrows():
                                                ...
                                                # 流動資産
                                                if any(x in item for x in ['現金', '預金', '売掛', '棚卸', '商品', '貯蔵', '前払', '立替', '未収', '仮払', '流動資産']):
                                                    if '合計' not in item:
                                                        current_assets += amt
                                                # 固定資産
                                                elif any(x in item for x in ['固定資産', '附属', '車両', '投資', '敷金', '差入', '保険積立', '長期']):
                                                    if '合計' not in item and '負債' not in item:
                                                        fixed_assets += amt
                                                # 流動負債
                                                elif any(x in item for x in ['買掛', '短期借入', '未払', '預り', '仮受', '流動負債']):
                                                    if '合計' not in item:
                                                        current_liabilities += amt
                                                # 固定負債
                                                elif '長期借入' in item or ('固定負債' in item and '合計' not in item):
                                                    fixed_liabilities += amt
                                                # 純資産
                                                elif any(x in item for x in ['資本', '剰余', '利益', '純資産']):
                                                    if '合計' not in item and '負債' not in item:
                                                        equity += amt
                                            
                                            bs_comparison_data.append({
                                                '期': f'第{period_id}期',
                                                '流動資産': current_assets,
                                                '固定資産': fixed_assets,
                                    ...
                                    if bs_comparison_data:
                                        # 積み上げ棒グラフ作成
                                        fig = go.Figure()
                                        
                                        categories = [d['期'] for d in bs_comparison_data]
                                        
                                        # 資産側（左）
                                        fig.add_trace(go.Bar(
                                            name='流動資産',
                                            x=categories,
                                            y=[d['流動資産'] for d in bs_comparison_data],
                                            marker_color='#00BCD4',
                                            text=[f"{d['流動資産']/10000:,.0f}" for d in bs_comparison_data],
                                            textposition='inside',
                                        ...
                                        fig.add_trace(go.Bar(
                                            name='固定資産',
                                            x=categories,
                                            y=[d['固定資産'] for d in bs_comparison_data],
                                            marker_color='#9E9E9E',
                                            text=[f"{d['固定資産']/10000:,.0f}" for d in bs_comparison_data],
                                            textposition='inside',
                                            textfont=dict(color='white', size=14, family='Arial Black'),
                                        ...
                                        fig.add_trace(go.Bar(
                                            name='流動負債',
                                            x=categories,
                                            y=[d['流動負債'] for d in bs_comparison_data],
                                            marker_color='#8BC34A',
                                            text=[f"{d['流動負債']/10000:,.0f}" for d in bs_comparison_data],
                                            textposition='inside',
                                            textfont=dict(color='white', size=14, family='Arial Black'),
                                        ...
                                        fig.add_trace(go.Bar(
                                            name='固定負債',
                                            x=categories,
                                            y=[d['固定負債'] for d in bs_comparison_data],
                                            marker_color='#4CAF50',
                                            text=[f"{d['固定負債']/10000:,.0f}" for d in bs_comparison_data],
                                            textposition='inside',
                                            textfont=dict(color='white', size=14, family='Arial Black'),
                                        ...
                                        fig.add_trace(go.Bar(
                                            name='純資産',
                                            x=categories,
                                            y=[d['純資産'] for d in bs_comparison_data],
                                            marker_color='#F44336',
                                            text=[f"{d['純資産']/10000:,.0f}" for d in bs_comparison_data],
                                            textposition='inside',
                                            textfont=dict(color='white', size=14, family='Arial Black'),
                                        ...
                                        fig.update_layout(
                                            barmode='stack',
                                            title={
                                                'text': f'{len(bs_comparison_data)}期比較貸借対照表',
